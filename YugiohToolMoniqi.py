...
class Moniqi:
	...
	def pvp_tolose_moniqi(self, arg):  # pvp自杀
		win32api.keybd_event(13, 0, 0, 0)  #
		win32gui.SetForegroundWindow(self.hwnd)  # 窗口显示最前面
		mode = None
		...
		while True:
			time.sleep(0.5)
			self.dianji_guding(0.5449, 0.0901)  # 点击空白
			...
			if zjm is not None:
				self.dianji_guding(0.3633, 0.9609)  # 点击pvp门固定点
				time.sleep(2)
				...
				if str(arg) == '1':
					...
					self.tuodong_guding(0.2283, 0.7645)
					time.sleep(2)
				...
				if mode is not None:
					...
					while True:
						...
						if jd is not None:
							...
							for i in range(50):
								time.sleep(0.5)
								self.dianji_guding(0.0415, 0.8176)  # 点击空白
								...
								if drw is not None:
									...
									if cd is not None:
										...
										if js is not None:
											...
											pyg.press('f2')  # 点击多任务键
											time.sleep(32)
											self.dianji_guding(0.4861, 0.8857)  # 点击游戏
											time.sleep(2)
											break
									else:
										pyg.press('f2')  # 点击多任务键
						...
						if qr is not None:
							...
							self.dianji_guding(0.173, 0.6713)
							time.sleep(0.2)
							self.dianji_guding(0.519, 0.6713)
							time.sleep(0.2)
	...
	def chuansongmen_moniqi(self):  # 十级门
		win32api.keybd_event(13, 0, 0, 0)  #
		win32gui.SetForegroundWindow(self.hwnd)  # 窗口显示最前面
		cf = configparser.ConfigParser()
		cf.read("config.ini")  # 读取配置文件
		...
		while True:
			...
			fhui = pyg.locateOnScreen('YugiohTool/api-fh.dll', confidence=0.80, grayscale=True)
			if zjm is not None:
				self.dianji_guding(0.1557, 0.9619)  # 点击传送门固定点
				time.sleep(5)
				...
				time.sleep(0.3)
				if sjd is not None:
					self.dianji_guding(0.1557, 0.6913)  # 点击等级:10
					possjd = sjd
					...
					try:
						for i in range(100):
							gongjicount = 0
							time.sleep(0.5)
							self.dianji_guding(0.0173, 0.8617)
							cd = pyg.locateOnScreen('YugiohTool/api-cd.dll', confidence=0.80, grayscale=True)
							haod = pyg.locateOnScreen('YugiohTool/api-h.dll', confidence=0.80, grayscale=True)
							if cd is not None:
								self.tuodong_guding(0.3979, 0.8717)  # 拖动卡牌
								time.sleep(0.5)
								self.dianji_guding(0.346, 0.7515)  # 点击通常召唤
								time.sleep(2)
								s_x, s_y = pyg.center(cd)
								self.dianji(s_x, s_y)  # 点击菜单
								time.sleep(1)
								...
								if zdu is not None:
									...
									self.dianji_guding(0.7179, 0.6833)  # 点击结束回合
									huihecount = huihecount + 1
							if haod is not None:
								haod_x, haod_y = pyg.center(haod)
								self.dianji(haod_x, haod_y)  # 点击好
	...
	def pvp_xiuxian_towin_moniqi(self):  # pvp休闲towin
		win32api.keybd_event(13, 0, 0, 0)  #
		win32gui.SetForegroundWindow(self.hwnd)  # 窗口显示最前面
		cf = configparser.ConfigParser()
		cf.read("config.ini")  # 读取配置文件
		...
		while True:
			...
			fhui = pyg.locateOnScreen('YugiohTool/api-fh.dll', confidence=0.80, grayscale=True)
			if zjm is not None:
				self.dianji_guding(0.3633, 0.9619)  # 点击pvp门固定点
				time.sleep(2)
				self.tuodong_guding(0.3979, 0.8717)
				time.sleep(2)
				xx = pyg.locateOnScreen('YugiohTool/api-xx.dll', confidence=0.80, grayscale=True)
				time.sleep(1)
				if xx is not None:
								...
								while a < 150:
									...
									if cd is not None:
										...
										if zdu is not None:
											...
											self.dianji_guding(0.692, 0.6813)  # 点击结束回合
											huihecount = huihecount + 1
									if haod is not None:
										haod_x, haod_y = pyg.center(haod)
										self.dianji(haod_x, haod_y)  # 点击好
	...
	def pvp_paimin_towin_moniqi(self):  # pvp排名towin
		win32api.keybd_event(13, 0, 0, 0)  #
		win32gui.SetForegroundWindow(self.hwnd)  # 窗口显示最前面
		global tuisong_time
		cf = configparser.ConfigParser()
		...
		while True:
			...
			if zjm is not None:
				...
				if pm is not None:
					...
					while True:
						time.sleep(0.5)
						self.dianji_guding(0.5449, 0.0901)
						jd = pyg.locateOnScreen('YugiohTool/api-jd.dll', confidence=0.60, grayscale=True)
						haod = pyg.locateOnScreen('YugiohTool/api-h.dll', confidence=0.80, grayscale=True)
						xyb = pyg.locateOnScreen('YugiohTool/api-xyb.dll', confidence=0.80, grayscale=True)
						qr = pyg.locateOnScreen('YugiohTool/api-qr.dll', confidence=0.80, grayscale=True)
	...
	def huodong_zuidui_moniqi(self):
		win32api.keybd_event(13, 0, 0, 0)  #
		win32gui.SetForegroundWindow(self.hwnd)  # 窗口显示最前面
		while True:
			time.sleep(0.5)
			...
			fhui = pyg.locateOnScreen('YugiohTool/api-fh.dll', confidence=0.80, grayscale=True)
			if zjm is not None:
				self.dianji_guding(0.3287, 0.8917)  # 点击活动固定点
				time.sleep(2)
